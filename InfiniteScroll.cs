...
namespace Mopsicus.InfiniteScroll {
	...
	public class InfiniteScroll : MonoBehaviour, IDropHandler {
		...
		/// <summary>
		/// Item's default RectTransform pivot (2021-02-05)
		/// </summary>
		[Header("아이템 기본 Pivot : Scroll Content Rect 내 item 위치 설정)")]
		[SerializeField] private Vector2 defaultItemRectPivot = new Vector2(0, 1f);
		...
		/// </summary>
		public event WidthtItem OnWidth;
		...
		void CreateViewsVertical () {
			...
			for (int i = 0; i < fillCount; i++) {
				...
				rect.pivot = defaultItemRectPivot;  // Changed because items miss-positioned when scrolling (2021-02-05)
				...
				// Added because item size of width is set to 0 (2020-06-25)
				rect.sizeDelta = new Vector2(_content.sizeDelta.x, 0);
